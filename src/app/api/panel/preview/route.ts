...
const schema=z.object({
	...
	layout:z.object({blocks:z.array(z.enum(BLOCK_IDS)).min(1).max(MAX_BLOCKS),spans:z.record(z.string(),z.number().int().min(1).max(4)),photoDataUrl:z.string().max(1_500_000).regex(/^data:image\/(?:png|jpeg|webp);base64,/).optional()}).refine(value=>layoutFits(value as PanelLayout)),
})
