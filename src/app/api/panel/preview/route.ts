...
const schema=z.object({
	...
	layout:z.object({blocks:z.array(z.enum(BLOCK_IDS)).min(1).max(MAX_BLOCKS),spans:z.record(z.string(),z.number().int().min(1).max(4)),rowSpans:z.record(z.string(),z.number().int().min(1).max(2)).optional(),ranges:z.record(z.string(),z.enum(['day','days3','week','weeks2','month'])).optional(),sensorChartRange:z.enum(['hour','hours3','hours6','hours12','hours23','hours24','days3','week','month']).optional(),sensorChartFilter:z.enum(['raw','spikes','median']).optional(),photoDataUrl:z.string().max(1_500_000).regex(/^data:image\/(?:png|jpeg|webp);base64,/).optional(),fontSize:z.number().int().min(80).max(200).optional(),theme:z.enum(['classic','night','poster','air','rail']).optional(),cornerRadius:z.number().int().min(0).max(32).optional(),cardGap:z.number().int().min(0).max(28).optional(),showBorder:z.boolean().optional(),showFrame:z.boolean().optional(),cacheScreen:z.boolean().optional(),quietHours:z.object({enabled:z.boolean(),startHour:z.number().int().min(0).max(23),endHour:z.number().int().min(0).max(23),refreshMinutes:z.number().int().min(1).max(1440)}).optional(),sensor:z.object({pressure:z.boolean(),altitude:z.boolean(),humidity:z.boolean()}).optional(),header:z.object({visible:z.boolean(),showCity:z.boolean(),showCoords:z.boolean(),showDate:z.boolean(),showTime:z.boolean(),showBattery:z.boolean(),title:z.string().trim().max(48).optional(),style:z.enum(['fill','invert','line']),size:z.enum(['s','m','l'])}).optional()}).refine(value=>layoutFits(value as PanelLayout)),
})
